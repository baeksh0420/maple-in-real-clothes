...
class pixelate:
    ...
    def pixelate(img_path, mode=None, is_save=None, save_path=None):
        ...
        pyx = Pyx(factor=downsample_by, palette=palette, dither="naive", alpha=.6, upscale=4)
        ...
        if is_save=="save":
            print(save_path+"\\"+img_path.split("\\")[-1].split(".")[0]+".jpg")
            cv2.imwrite(save_path+"\\"+img_path.split("\\")[-1].split(".")[0]+".jpg", new_image)
    
    def pixelate_all(load_dir,save_dir, mode=None, is_save="save"):
        ...
        for img_str in tqdm(img_lst):
            ...
            pixelate.pixelate(img_path=new_path, mode=mode, is_save=is_save, save_path=save_path)
